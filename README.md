...
本项目已接入火山方舟官方 OpenAI 兼容 API。请先在本地 `.env` 配置密钥，然后启动本地代理服务：
...
注意：不要用 `python -m http.server` 启动真实模型版本，因为它不会提供 `/api/agent-chat` 代理接口。
...
- **群聊调度**：输入复杂任务后，本地代理会调用火山方舟模型，由模型生成多 Agent 协作回复。
...
真实 API Key 不应提交到仓库。请把挑战赛提供的密钥放在本地 `.env` 或平台密钥管理中，并参考 `.env.example` 命名。前端不会直接读取密钥，所有模型请求都通过 `server.py` 转发。
